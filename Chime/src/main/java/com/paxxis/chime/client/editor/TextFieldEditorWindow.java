 ...
 */
public class TextFieldEditorWindow extends ChimeWindow
{
    private FieldEditorListener _listener;
    
    private SimpleTextEditorPanel _simpleEditorPanel = null;
    private ChimeRichTextEditor _editorPanel = null;
    ...
    public TextFieldEditorWindow(Shape type, DataField field, DataFieldValue value, FieldEditorListener listener)
    {
        _type = type;
        _field = field;
        _value = value;
    ...
    protected void init()
    {
        ...
        simpleText = _field.getShape().getName().equals("Text");

        setHeading("Edit " + _field.getName() + " [" + _type.getName() + "]");

        if (_value == null) {
            if (simpleText) {
                _simpleEditorPanel = new SimpleTextEditorPanel("");
            } else {
                _editorPanel = new ChimeRichTextEditor("", true);
            }
        } else {
            if (simpleText) {
    ...
    private void notifyComplete()
    {
        ...
        if (simpleText) {
            text = _simpleEditorPanel.getText();
        } else {
            text = _editorPanel.getHTML();
        }
        
        _listener.onSave(_field, _value, text);
    }
    ...
    private void validate()
    {
        ...
        if (simpleText) {
            if (_simpleEditorPanel.getText().trim().length() == 0)
            {
