 ...
 */
public class CreateDiscussionResponseHandler extends ResponseHandler<CreateDiscussionResponse>
{
    private static MessageType _supportedType = CreateDiscussionResponse.messageType();
    private static int _supportedVersion = CreateDiscussionResponse.messageVersion();
    ...
    protected SimpleMessageProcessor getProcessor(int type, int version, int payloadType)
    {
        ...
        {
            return new CreateDiscussionResponseProcessor(new JavaObjectPayload(), getResponseListener());
        }
