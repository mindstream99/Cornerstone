 ...
 */
public class EditInstanceResponseHandler extends ResponseHandler<EditDataInstanceResponse>
{
    private static MessageType _supportedType = EditDataInstanceResponse.messageType();
    private static int _supportedVersion = EditDataInstanceResponse.messageVersion();
    ...
    protected SimpleMessageProcessor getProcessor(int type, int version, int payloadType)
    {
        ...
        {
            return new EditInstanceResponseProcessor(new JavaObjectPayload(), getResponseListener());
        }
