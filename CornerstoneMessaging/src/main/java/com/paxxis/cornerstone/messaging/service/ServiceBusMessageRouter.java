...
public class ServiceBusMessageRouter extends ServiceBusMessageHandler {
    ...
    public MessageProcessor<?, ?> getProcessor(int type, int version, int payloadType) {
        ...
        if (payloadType != processor.getPayloadType().getValue()) {
            throw new RuntimeException("Invalid Payload type detected, expected " + processor.getPayloadType() + ", actual " + payloadType);
        }
